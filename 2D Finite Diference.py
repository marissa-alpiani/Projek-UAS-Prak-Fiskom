...
# Kondisi batas
u[0, :] = 0  # (a) Suhu tepi kiri
u[-1, :] = 100  # (b) Suhu tepi kanan
u[:, 0] = np.linspace(0, 100, node)  # (c) Suhu tepi bawah (variasi linear)
u[:, -1] = np.linspace(0, 100, node)  # (d) Suhu tepi atas (variasi linear)
...
# Konfigurasi plot yang akan ditampilkan
counter = 0
...
while counter < waktu:
    ...
    plt.pause(0.01)  # Menambah jeda untuk memperbarui plot
    counter += dt

# Menampilkan plot setelah simulasi selesai
plt.show()
